...
import { CHAIN_INFO, L2ChainInfo, L2_CHAIN_IDS, SupportedChainId, SupportedL2ChainId } from '../../constants/chains';
...
export default function NetworkCard() {
  ...
  const l1info = chainId ? CHAIN_INFO[chainId] : undefined;
  if (!chainId || chainId === SupportedChainId.MAINNET || !l1info || !library) {
    return null;
  }

  if (L2_CHAIN_IDS.includes(chainId)) {
    const info = CHAIN_INFO[chainId as SupportedL2ChainId] as unknown as L2ChainInfo;
  ...
  return <FallbackWrapper title={l1info.label}>{l1info.label}</FallbackWrapper>;
}
